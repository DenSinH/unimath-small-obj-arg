...
Definition right_lifting_data (g : arrow C) : UU :=
    ∏ (f : arrow C), (J _ _ (arrow_mor f)) -> ∏ (S : lifting_problem f g), filler S.
...
Definition nwfs_R_maps_disp (n : nwfs C) :=
    Algebra_disp (nwfs_R_monad n).
Definition nwfs_L_maps_disp (n : nwfs C) :=
    Algebra_disp (nwfs_L_monad n).

Context (n : nwfs C).

Definition morcls_L_map_structure : UU := 
  disp_functor (functor_identity _) (op_disp_cat morcls_disp) (nwfs_L_maps_disp n).

(* "this assignation extends to a functor θ:", Garner 2007, p18 *)
Definition rlp_morcls_R_map_structure (η : morcls_L_map_structure) : 
    disp_functor (functor_identity _) (nwfs_R_maps_disp n) (rlp_morcls_disp).
Proof.
  use tpair.
  - use tpair.
    * intros x hx.
